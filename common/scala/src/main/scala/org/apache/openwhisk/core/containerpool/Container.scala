...
import org.apache.openwhisk.core.hyfaas.HyfaasActivationContext
...
trait Container {
  ...
  def run(parameters: JsValue,
    ...
    val hyfaasEnvironment = HyfaasActivationContext
      .fromParameters(parameters)
      .map(HyfaasActivationContext.inject(environment, _))
      .getOrElse(environment)
    val body = JsObject(parameterWrapper.fields ++ hyfaasEnvironment.fields)
