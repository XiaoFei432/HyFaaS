...
import org.apache.openwhisk.core.hyfaas.{HyfaasRequest, HyfaasService}
...
class Controller(val instance: ControllerInstanceId,
  ...
  protected[controller] val hyfaasRoutes = {
    import org.apache.openwhisk.core.hyfaas.HyfaasJsonProtocol._

    (pathPrefix("hyfaas") & post) {
      path("plan") {
        entity(as[HyfaasRequest]) { request =>
          complete(HyfaasService.plan(request))
        }
      }
    }
  }
  ...
  private def adminRoutes(implicit transid: TransactionId) = {
    sendCorsHeaders {
      options {
        complete(OK)
      } ~ internalInvokerHealth ~ activationStatus ~ hyfaasRoutes ~ disable
    }
  }
}
